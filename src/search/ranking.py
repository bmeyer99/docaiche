...
class ResultRanker:
    ...
    async def rank_results(
        ...
        """
        try:
            logger.info(f"Ranking {len(results)} search results using strategy: {strategy_used}")
            ...
            ranked_results = sorted(scored_results, key=lambda r: r.relevance_score, reverse=True)
            
            # Step 3: Apply position-based adjustments
            final_results = self._apply_position_adjustments(ranked_results)
            
            logger.info(f"Ranking completed: top score = {final_results[0].relevance_score:.3f}")
    ...
    async def _calculate_final_score(
        ...
        vector_score = result.relevance_score * self.scoring_weights['vector_similarity']
        ...
        quality_score = (result.quality_score or 0.5) * self.scoring_weights['quality_score']
        
        # Calculate technology matching boost
        tech_score = self._calculate_technology_score(
            result, technology_hint
        ) * self.scoring_weights['technology_match']
        
        # Combine all factors
        final_score = vector_score + metadata_score + recency_score + quality_score + tech_score
    ...
    def _apply_strategy_adjustments(self, score: float, strategy: SearchStrategy) -> float:
        """
    ...
    def _apply_position_adjustments(self, results: List[SearchResult]) -> List[SearchResult]:
        """
    ...
    def deduplicate_results(
        ...
        """
        try:
            logger.info(f"Deduplicating {len(results)} results with threshold {dedup_threshold}")
            ...
            for content_id, group in content_groups.items():
                if len(group) == 1:
                    ...
                    logger.debug(f"Deduplicated {len(group)} results for content_id: {content_id}")
            
            logger.info(f"Deduplication completed: {len(results)} -> {len(deduplicated)} results")
            return deduplicated
