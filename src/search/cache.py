...
class SearchCacheManager:
    ...
    async def cache_results(
        ...
        try:
            ...
            logger.info(f"Cached search results: {len(results.results)} results, TTL: {ttl}s")
    ...
    async def invalidate_cache(self, pattern: Optional[str] = None) -> int:
        ...
        try:
            if pattern:
                ...
                logger.warning("Full cache invalidation not implemented - would require Redis SCAN")
                return 0
    ...
    async def _update_access_stats(self, cache_key: str, cached_result: CachedSearchResult) -> None:
        """
