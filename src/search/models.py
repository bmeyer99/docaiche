...
class SearchQuery(BaseModel):
    ...
    strategy: SearchStrategy = Field(SearchStrategy.HYBRID, description="Search strategy to use")
...
class SearchResults(BaseModel):
    ...
    strategy_used: SearchStrategy = Field(..., description="Search strategy that was executed")
    cache_hit: bool = Field(False, description="Whether results came from cache")
    workspaces_searched: List[str] = Field(default_factory=list, description="Workspaces included in search")
    enrichment_triggered: bool = Field(False, description="Whether enrichment was triggered")
...
class WorkspaceInfo(BaseModel):
    ...
    document_count: Optional[int] = Field(None, description="Number of documents in workspace")
...
class EvaluationResult(BaseModel):
    ...
    """
    overall_quality: float = Field(..., ge=0.0, le=1.0, description="Overall result quality score")
    relevance_assessment: float = Field(..., ge=0.0, le=1.0, description="Relevance assessment score")
    completeness_score: float = Field(..., ge=0.0, le=1.0, description="Answer completeness score")
    needs_enrichment: bool = Field(..., description="Whether enrichment is recommended")
    enrichment_topics: List[str] = Field(default_factory=list, description="Suggested enrichment topics")
    confidence_level: float = Field(..., ge=0.0, le=1.0, description="Evaluation confidence level")
...
class SearchAnalytics(BaseModel):
    ...
    metadata_search_time_ms: Optional[int] = Field(None, description="Metadata search time")
    ...
    enrichment_triggered: bool = Field(..., description="Whether enrichment was triggered")
