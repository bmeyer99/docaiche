...
from src.web_ui.api_gateway.schemas import (
    HealthResponse,
    StatsResponse,
    ContentResponse,
)
...
async def get_config(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch system configuration"
        )
...
async def update_config(
    ...
    try:
        ...
        if not os.getenv("TESTING") == "1":
            ...
            req_csrf = request.headers.get("x-csrf-token") or request.cookies.get(
                "csrf"
            )
...
async def reset_config(
    ...
    try:
        ...
        return {
            ...
            "config": (
                reset_config.model_dump()
                if hasattr(reset_config, "model_dump")
                else reset_config
            ),
        }
        ...
        raise HTTPException(
            status_code=500, detail="Failed to reset system configuration"
        )


@api_router.get("/content", response_model=ContentResponse)
async def get_content(
    view_model_service: ViewModelService = Depends(get_view_model_service),
):
...
async def delete_content(
    content_id: str, data_service: DataService = Depends(get_data_service)
):
...
async def admin_search_content_post(
    req: SearchContentRequest, data_service: DataService = Depends(get_data_service)
):
        ...
        if req.provider.lower() == "ollama":
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"Successfully fetched {len(formatted_models)} models from Ollama"
                    )
                    return {
                        "success": True,
    ...
    try:
        ...
        if req.provider.lower() == "ollama":
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                logger.info(
                    f"Testing model {req.model} with prompt: {req.prompt[:50]}..."
                )
                ...
                if response.status_code == 200:
                    ...
                    error_text = response.text
                    logger.error(
                        f"Ollama model test failed: HTTP {response.status_code}: {error_text}"
                    )
                    return {
                        "success": False,
