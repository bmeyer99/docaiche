...
class ContentService:
    ...
    async def ingest_content(
        self, request: ContentIngestionRequest
    ) -> ContentIngestionResponse:
        """
        ...
        if not self.db_manager:
            return ContentIngestionResponse(
                content_id="", status="failed", message="Database manager not available"
            )
        ...
        async with self.db_manager.transaction():
            try:
                ...
                WHERE source_url = :source_url AND workspace = :workspace
                """
    ...
    async def get_content_stats(self) -> ContentStatsResponse:
        ...
        try:
            ...
            by_content_type = {
                row["content_type"]: row["count"] for row in type_results
            }
