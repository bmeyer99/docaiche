...
class SecretsManager:
    ...
    def get_required_secret(env_var: str, description: str) -> str:
        ...
        if not value:
            raise ValueError(
                f"Required secret {env_var} not found in environment: {description}"
            )
        return value
    ...
    def validate_production_secrets(config) -> None:
        ...
        if config.app.environment == "production":
            ...
            if (
                ...
                or config.ai.fallback_provider == "openai"
            ...
            if required_secrets:
                raise ValueError(
                    f"Missing required secrets for production: {', '.join(required_secrets)}"
                )
