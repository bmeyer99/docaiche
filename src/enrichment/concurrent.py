...
from .exceptions import EnrichmentTimeoutError, QueueError, TaskExecutionError
        ...
        try:
            ...
            async with self._resource_lock:
                ...
                self._metrics["max_concurrent"] = max(
                    self._metrics["max_concurrent"], self._metrics["current_active"]
                )
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        async with self._resource_lock:
            return {
                ...
                "utilization_percent": (len(self._active_resources) / self.max_size)
                * 100,
...
class ConcurrentTaskExecutor:
    ...
    def __init__(
        self, config: EnrichmentConfig, resource_limits: Optional[ResourceLimits] = None
    ):
        """
        ...
        self.max_concurrent_tasks = getattr(
            config, "max_concurrent_tasks", self.resource_limits.max_processing_slots
        )
    ...
    async def execute_task(
        ...
        try:
            ...
            await self._deadlock_detector.check_potential_deadlock(
                task_id, required_resources
            )
            ...
            async with self._isolation_manager.isolated_execution(
                task_id, {"task_type": task.task_type, "priority": task.priority}
            ):
                ...
                async with self._acquire_resources(task_id, required_resources):
                    ...
                    result = await asyncio.wait_for(
                        task_handler(task), timeout=self.config.task_timeout_seconds
                    )
            ...
            async with self._task_state_lock:
                ...
                task.error_message = (
                    f"Task timed out after {self.config.task_timeout_seconds}s"
                )
                task.completed_at = datetime.utcnow()
    ...
    async def _acquire_resources(
        self, task_id: str, resource_types: List[ResourceType]
    ):
        """
        ...
        try:
            ...
            for context in reversed(acquired_contexts):
                try:
                    ...
                    logger.error(
                        f"Cleanup error during exception handling: {cleanup_error}"
                    )
            raise
        ...
        try:
            ...
            for resource_type, pool in self._resource_pools.items():
                ...
                if health.get("status") != "healthy":
                    logger.warning(
                        f"Resource pool '{resource_type.value}' health check: {health}"
                    )
    ...
    def _generate_health_recommendations(self, metrics: Dict[str, Any]) -> List[str]:
        ...
        if metrics["priority_queue"]["at_capacity"]:
            recommendations.append(
                "Priority queue at capacity - consider increasing processing capacity"
            )
        ...
        if error_rate > 5:
            recommendations.append(
                f"High error rate ({error_rate:.1f}%) - investigate task failures"
            )

        # Check deadlock prevention
        if metrics["deadlock_detector"]["potential_deadlocks"] > 0:
            recommendations.append(
                "Potential deadlocks detected - review task dependencies"
            )
...
class DeadlockDetector:
    ...
    async def check_potential_deadlock(
        self, task_id: str, required_resources: List[ResourceType]
    ) -> None:
        """
    ...
    async def _detect_circular_wait(
        self, task_id: str, resource_names: List[str]
    ) -> bool:
        """
