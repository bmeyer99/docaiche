...
class KnowledgeEnricher:
    ...
    def __init__(
        ...
        # Lifecycle manager
        self.lifecycle_manager = SimpleEnrichmentLifecycleManager(db_manager, config.__dict__ if hasattr(config, '__dict__') else {})
        
        # Task management
        from src.enrichment.tasks import TaskManager
        self.task_manager = TaskManager(config=config.__dict__ if hasattr(config, '__dict__') else {})
    ...
    def analyze_content_gaps(self, query: str) -> List[Dict[str, Any]]:
        ...
        for pattern in dangerous_patterns:
            if re.search(pattern, query, re.IGNORECASE | re.DOTALL):
                raise ValueError("Query contains potentially dangerous content and was blocked for security")
    ...
    async def enrich(self, task: EnrichmentTask) -> None:
        ...
        try:
            ...
            gap_analyzer = SimpleGapAnalyzer(self.db_manager, self.config.__dict__ if hasattr(self.config, '__dict__') else {})
    ...
    async def enrich_content(self, content_id: str) -> Dict[str, Any]:
        ...
        if not self._running:
            raise Exception("KnowledgeEnricher must be started before enriching content")
        
        # Create enrichment task
        task = EnrichmentTask(
            task_id=f"enrich_{content_id}",
...
class EnrichmentWorkflow:
    ...
    async def run(self, task: EnrichmentTask) -> None:
        ...
        try:
            ...
            query = "UPDATE enrichment_tasks SET status = :param_0 WHERE task_id = :param_1"
