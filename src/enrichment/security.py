        ...
        try:
            ...
            if hasattr(resource, "RLIMIT_AS"):
                ...
                if (
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Cannot set memory limit in container environment: {e}"
                        )
                        # Clear this limit from restoration list
                        self._original_limits.pop("memory", None)
                else:
                    logger.debug(
                        f"Current memory limit ({current_limit}) already restrictive"
                    )
            ...
            if hasattr(resource, "RLIMIT_CPU"):
                ...
                if (
                    ...
                    or current_cpu_limit == -1
                    or current_cpu_limit > self.max_cpu_time_seconds
                ):
                    try:
                        resource.setrlimit(
                            resource.RLIMIT_CPU,
                        ...
                        logger.debug(
                            f"CPU time limit set to {self.max_cpu_time_seconds}s"
                        )
                    except (ValueError, PermissionError) as e:
                        logger.warning(
                            f"Cannot set CPU limit in container environment: {e}"
                        )
                        self._original_limits.pop("cpu", None)
                else:
                    logger.debug("Current CPU limit already restrictive")
            ...
            if hasattr(resource, "RLIMIT_FSIZE"):
                ...
                if (
                    current_fsize_limit == resource.RLIM_INFINITY
                    or current_fsize_limit == -1
                    or current_fsize_limit > file_size_limit
                ):
                    try:
                        resource.setrlimit(
                        ...
                        logger.debug(
                            f"File size limit set to {self.max_file_size_mb}MB"
                        )
                    except (ValueError, PermissionError) as e:
                        logger.warning(
                            f"Cannot set file size limit in container environment: {e}"
                        )
                        self._original_limits.pop("fsize", None)
                else:
                    logger.debug("Current file size limit already restrictive")
        ...
        try:
            ...
            for limit_type, original_limit in self._original_limits.items():
                try:
                    ...
                    should_restore = (
                        original_limit[0] != resource.RLIM_INFINITY
                        and original_limit[0] != -1
                    )
                    ...
                    if should_restore:
                        ...
                        elif limit_type == "fsize" and hasattr(
                            resource, "RLIMIT_FSIZE"
                        ):
                            resource.setrlimit(resource.RLIMIT_FSIZE, original_limit)
                    else:
                        logger.debug(
                            f"Skipping restore of unlimited {limit_type} limit in container"
                        )

                except (ValueError, PermissionError) as e:
                    # Container environments may not allow limit restoration
                    logger.debug(
                        f"Cannot restore {limit_type} limit in container environment: {e}"
                    )
                except Exception as e:
...
class SecureTaskExecutor:
    ...
    async def _execute_with_monitoring(
        ...
        try:
            ...
            logger.debug(
                f"Task executed in {execution_time:.2f}s with privilege level: {sandbox.privilege_level}"
            )
...
async def secure_task_context(
    task_id: str, privilege_level: str = PrivilegeLevel.RESTRICTED
):
    """
