...
class LifecycleManager:
    ...
    def _setup_component_dependencies(self) -> None:
        ...
        self._startup_order = ['database', 'task_queue', 'concurrent_executor', 'task_manager', 'knowledge_enricher']
    ...
    def _setup_signal_handlers(self) -> None:
        ...
        if hasattr(signal, 'SIGTERM'):
            self._signal_handlers[signal.SIGTERM] = lambda s, f: asyncio.create_task(signal_handler(s))
            self._signal_handlers[signal.SIGINT] = lambda s, f: asyncio.create_task(signal_handler(s))
    ...
    async def _create_component(self, component_name: str) -> Any:
        ...
        if component_name == 'database':
            ...
            return TaskManager(config=self.config.__dict__ if hasattr(self.config, '__dict__') else {})
    ...
    async def _validate_all_dependencies(self) -> None:
        ...
        for component_name, dependencies in self._dependency_map.items():
            for dependency_name in dependencies:
                validation = await self._validate_dependency(component_name, dependency_name)
    ...
    async def _validate_dependency(self, component: str, dependency: str) -> DependencyValidation:
        ...
        try:
            ...
            return DependencyValidation(
                ...
                validation_time_ms=(time.time() - start_time) * 1000,
                error_message=None if is_healthy else f"Dependency '{dependency}' is unhealthy"
            )
    ...
    async def start_all_components(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"All components started successfully in {startup_time:.2f}s")
            ...
            for component_name in reversed(started_components):
                try:
                    ...
                    self.logger.error(f"Error stopping component during cleanup: {cleanup_error}")
        ...
        for component_name in stop_order:
            if component_name in self._components:
                try:
                    ...
                    self.logger.warning(f"Component '{component_name}' shutdown timed out")
                    component_results[component_name] = {'status': 'timeout'}
                except Exception as e:
                    self.logger.error(f"Error stopping component '{component_name}': {e}")
                    component_results[component_name] = {'status': 'error', 'error': str(e)}
        
        self._running = False
        self._lifecycle_metrics['total_shutdowns'] += 1
        
        shutdown_time = time.time() - start_time
        graceful = all(r.get('status') in ['stopped', 'already_stopped'] for r in component_results.values())
    ...
    async def _recover_component(self, component_name: str) -> None:
        ...
        try:
            ...
            if restart_result['status'] == 'success':
                ...
                raise Exception(f"Restart failed: {restart_result.get('error', 'Unknown error')}")
