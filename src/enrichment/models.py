...
class EnrichmentTask(BaseModel):
    ...
    task_type: EnrichmentType = Field(
        default=EnrichmentType.CONTENT_ANALYSIS, description="Type of enrichment task"
    )
    priority: EnrichmentPriority = Field(
        default=EnrichmentPriority.NORMAL, description="Task priority level"
    )
    status: TaskStatus = Field(
        default=TaskStatus.PENDING, description="Current status of the enrichment task"
    )
    parameters: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Additional parameters for enrichment"
    )
...
class GapAnalysisResult(BaseModel):
    ...
    missing_topics: List[str] = Field(
        ...
        description="List of missing or insufficiently covered topics",
    )
    outdated_sections: List[str] = Field(
        default_factory=list, description="List of outdated sections"
    )
    recommendations: List[str] = Field(
        default_factory=list, description="Recommended actions to fill gaps"
    )
    confidence_score: float = Field(
        default=0.0, description="Confidence score for gap analysis"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Additional metadata for gap analysis result"
    )
...
class EnrichmentResult(BaseModel):
    ...
    results: Dict[str, Any] = Field(
        default_factory=dict, description="Enrichment results data"
    )
    confidence_score: float = Field(
        default=0.0, description="Confidence score for enrichment results"
    )
    processing_time_ms: Optional[float] = Field(
        None, description="Processing time in milliseconds"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Additional metadata for enrichment result"
    )
    created_at: Optional[datetime] = Field(
        None, description="Result creation timestamp"
    )
...
class EnrichmentConfig(BaseModel):
    ...
    max_concurrent_tasks: int = Field(
        default=5, description="Maximum number of concurrent enrichment tasks"
    )
    ...
    queue_poll_interval: int = Field(
        default=10, description="Queue polling interval in seconds"
    )
    batch_size: int = Field(default=10, description="Batch processing size")
    enable_relationship_mapping: bool = Field(
        default=True, description="Enable relationship mapping enrichment"
    )
    enable_tag_generation: bool = Field(
        default=True, description="Enable tag generation enrichment"
    )
    enable_quality_assessment: bool = Field(
        default=True, description="Enable quality assessment enrichment"
    )
    min_confidence_threshold: float = Field(
        default=0.7, description="Minimum confidence threshold for results"
    )
...
class QueueMetrics(BaseModel):
    ...
    last_updated: Optional[datetime] = Field(
        None, description="Last metrics update timestamp"
    )
...
class EnrichmentStrategy(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="A brief description of what the strategy does."
    )
    enabled: bool = Field(True, description="Whether the strategy is currently active.")
    parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Configuration parameters for the strategy."
    )
...
class ContentGap(BaseModel):
    ...
    confidence: float = Field(
        ..., description="Confidence score for gap identification"
    )
    suggested_sources: List[str] = Field(
        default_factory=list, description="Suggested sources to fill the gap"
    )
    priority: str = Field(..., description="Priority level for addressing the gap")
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional gap metadata"
    )
...
class EnrichmentMetrics(BaseModel):
    ...
    total_tasks_processed: int = Field(
        default=0, description="Total enrichment tasks processed"
    )
    ...
    average_processing_time_ms: float = Field(
        default=0.0, description="Average task processing time"
    )
...
class EnrichmentAnalytics(BaseModel):
    ...
    relationships_identified: int = Field(
        ..., description="Total relationships identified"
    )
    average_quality_improvement: float = Field(
        ..., description="Average quality score improvement"
    )
