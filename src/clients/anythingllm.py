...
class AsyncRequestContext:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if (
            self.response
            and hasattr(self.response, "closed")
            and not self.response.closed
        ):
            self.response.close()
    ...
    try:
        if hasattr(response, "status"):
            ...
            if hasattr(status, "_mock_return_value"):
                ...
                if (
                    hasattr(response, "_mock_children")
                    and "status" in response._mock_children
                ):
...
class AnythingLLMClient:
    ...
    async def get_or_create_workspace(
        ...
        try:
            async with self._make_request(
                "GET", f"/api/workspace/{workspace_slug}"
            ) as response:
        ...
        async with self._make_request(
            "POST", "/api/workspace/new", json=workspace_data
        ) as response:
            return await _safe_extract_json(response)
        ...
        async def upload_chunk(chunk: DocumentChunk) -> bool:
            async with semaphore:
                return await self._upload_single_chunk(
                    workspace_slug, document, chunk, upload_results
                )
    ...
    async def _upload_single_chunk(
        ...
        for attempt in range(max_retries):
            try:
                ...
                async with self._make_request(
                    "POST",
                    f"/api/workspace/{workspace_slug}/upload-text",
                    json=chunk_data,
                ) as response:
                    response_status = _safe_extract_status(response)
                    duration_ms = (time.time() - start_time) * 1000
                    ...
                    if response_status in [200, 201]:
                        ...
                        logger.warning(
                            f"Chunk upload failed with status {response_status}: {chunk.id}"
                        )
                        # Let the exception handling in _make_request deal with this
                        # But if we get here somehow, treat as failure
                        if attempt == max_retries - 1:
                            results["errors"].append(
                                f"Chunk {chunk.id}: HTTP {response_status}"
                            )
                        break
                    else:
                        logger.warning(
                            f"Chunk upload unexpected status {response_status}: {chunk.id}"
                        )
                        if attempt == max_retries - 1:
                            results["errors"].append(
                                f"Chunk {chunk.id}: Unexpected status {response_status}"
                            )

            except Exception as e:
                logger.warning(
                    f"Chunk upload attempt {attempt + 1} failed for {chunk.id}: {e}"
                )
    ...
    async def list_workspace_documents(
        ...
        async with self._make_request(
            "GET", f"/api/workspace/{workspace_slug}/list"
        ) as response:
            data = await _safe_extract_json(response)
    ...
    async def delete_document(self, workspace_slug: str, document_id: str) -> bool:
        ...
        try:
            async with self._make_request(
                "DELETE", f"/api/workspace/{workspace_slug}/delete/{document_id}"
            ) as response:
                response_status = _safe_extract_status(response)
            ...
            try:
                ...
                if status >= 400:
                    ...
                    if status == 401:
                        ...
                        elif hasattr(response, "headers") and isinstance(
                        ...
                        workspace_slug = (
                            endpoint.split("/")[-1] if "/" in endpoint else None
                        )
                ...
                raise AnythingLLMConnectionError(
                    ...
                    error_context={
                        "timeout": self.config.circuit_breaker.timeout_seconds
                    },
                )
