...
class IngestionPipeline:
    ...
    async def ingest_single_document(
        self, file_data: bytes, upload_request: DocumentUploadRequest
    ) -> IngestionResult:
        """
        ...
        try:
            logger.info(
                f"Starting document ingestion: {upload_request.filename} ({document_id})"
            )
            ...
            extracted_text, extraction_metadata = (
                await self.document_extractor.extract_content(
            ...
            file_content = FileContent(
                ...
                or f"upload://{upload_request.filename}",
                title=upload_request.title or upload_request.filename,
            )

            # Step 4: Process through Content Processing Pipeline
            processed_doc, status_message = (
                await self.content_processor.process_and_store_document(
            ...
            logger.info(
                f"Document ingestion completed: {document_id} in {processing_time:.0f}ms"
            )
            return result
    ...
    async def ingest_batch_documents(
        ...
        logger.info(
            f"Starting batch ingestion: {len(files_data)} documents ({batch_id})"
        )

        # Validate batch size
        if len(files_data) > self.max_batch_size:
            raise ValueError(
                f"Batch size {len(files_data)} exceeds maximum {self.max_batch_size}"
            )
        ...
        logger.info(
            f"Batch ingestion completed: {batch_id} - {len(successful_results)}/{len(files_data)} successful"
        )
        return batch_result
    ...
    async def get_processing_metrics(self) -> ProcessingMetrics:
        ...
        try:
            ...
            format_distribution = (
                {row[0]: row[1] for row in format_results} if format_results else {}
            )
            ...
            tech_distribution = (
                {row[0]: row[1] for row in tech_results} if tech_results else {}
            )
            ...
            success_rate = (
                (total_count / (total_count + failed_count)) * 100
            ...
            quality_distribution = (
                {row[0]: row[1] for row in quality_results} if quality_results else {}
            )
    ...
    async def _validate_file_security(
        ...
        if len(file_data) > self.max_file_size:
            raise ValueError(
                f"File size {len(file_data)} bytes exceeds limit {self.max_file_size}"
            )
        ...
        filename_ext = (
            upload_request.filename.lower().split(".")[-1]
        ...
        if (
            b"<script" in file_data.lower()[:1024]
        ):  # Basic script detection in first 1KB
            logger.warning(
                f"Potential script content detected in {upload_request.filename}"
            )
