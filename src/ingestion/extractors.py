...
try:
    ...
    logger.warning(
        "python-docx not available - Word document extraction will be disabled"
    )
...
class DocumentExtractor:
    ...
    async def extract_content(
        ...
        try:
            ...
            raise ValueError(
                f"Failed to extract content from {file_extension} file: {str(e)}"
            )
    ...
    async def _extract_word(
        self, file_data: bytes, format_type: str
    ) -> Tuple[str, Dict[str, Any]]:
    ...
    async def _extract_markdown(self, file_data: bytes) -> Tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            metadata = {"format": "md", "extraction_method": "markdown"}
