...
class JSONParser:
    ...
    @staticmethod
    def extract_and_parse(response_text: str, model_class: Type[T]) -> T:
        """
        ...
        """
        try:
            # Log the extraction attempt (mask sensitive data)
            ...
            try:
                ...
                raise JSONParsingError(f"Failed to parse JSON: {str(e)}")
            ...
            try:
                ...
                raise JSONValidationError(f"JSON doesn't match expected schema: {str(e)}")
                
        except Exception as e:
            if isinstance(e, (JSONParsingError, JSONValidationError)):
                raise
            logger.error(f"Unexpected error in JSON extraction: {e}")
            raise JSONParsingError(f"Unexpected parsing error: {str(e)}")
    ...
    def _fix_missing_quotes(text: str) -> str:
        ...
        # This is a simple heuristic - quote bare words followed by colons
