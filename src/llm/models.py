...
class EvaluationResult(BaseModel):
    ...
    relevance_score: float = Field(
        ..., ge=0.0, le=1.0, description="Relevance score for results"
    )
    ...
    sufficiency_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Sufficiency score"
    )
    confidence: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Confidence score"
    )
    missing_aspects: Optional[List[str]] = Field(
        default=None, description="Missing aspects"
    )
...
class EnrichmentStrategy(BaseModel):
    ...
    target_repositories: List[RepositoryTarget] = Field(
        default_factory=list, description="List of repository targets"
    )
    search_terms: List[str] = Field(
        default_factory=list, description="Search terms for enrichment"
    )
    confidence_score: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence score for this strategy"
    )
...
class QualityAssessment(BaseModel):
    ...
    content_quality: float = Field(
        ..., ge=0.0, le=1.0, description="Content quality score"
    )
...
def get_provider_status(config: Any) -> LLMProviderStatus:
    ...
    ollama = getattr(config, "ollama", None) or (
        config.get("ollama") if isinstance(config, dict) else None
    )
    openai = getattr(config, "openai", None) or (
        config.get("openai") if isinstance(config, dict) else None
    )
    primary = getattr(config, "primary_provider", None) or (
        config.get("primary_provider") if isinstance(config, dict) else None
    )
    fallback = getattr(config, "fallback_provider", None) or (
        config.get("fallback_provider") if isinstance(config, dict) else None
    )
