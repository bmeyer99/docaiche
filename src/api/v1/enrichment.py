...
from src.enrichment.models import (
    EnrichmentType, EnrichmentPriority, EnrichmentStrategy, 
    EnrichmentResult, EnrichmentMetrics, EnrichmentTask,
    ContentGap, EnrichmentAnalytics
)
...
class EnrichmentRequest(BaseModel):
    ...
    enrichment_types: Optional[List[EnrichmentType]] = Field(None, description="Types of enrichment to perform")
...
class BulkEnrichmentRequest(BaseModel):
    ...
    enrichment_type: EnrichmentType = Field(..., description="Type of enrichment to perform")
    priority: EnrichmentPriority = Field(EnrichmentPriority.LOW, description="Task priority")
...
class BulkImportRequest(BaseModel):
    ...
    technology_name: str = Field(..., description="Name of technology to import documentation for")
...
class EnrichmentStatusResponse(BaseModel):
    ...
    enrichment_available: bool = Field(False, description="Whether enrichment is available")
...
class EnrichmentSubmissionResponse(BaseModel):
    ...
    enrichment_types: List[str] = Field(..., description="Types of enrichment submitted")
...
class BulkEnrichmentResponse(BaseModel):
    ...
    task_ids: List[str] = Field(..., description="List of submitted task IDs")
    failures: List[Dict[str, Any]] = Field(..., description="List of failed submissions")
...
async def enrich_content(
    ...
    try:
        ...
        response = EnrichmentSubmissionResponse(
            task_ids=task_ids,
            content_id=request.content_id,
            enrichment_types=[t.value for t in request.enrichment_types] if request.enrichment_types else [],
            priority=request.priority.value,
            submitted_at=datetime.utcnow()
        )
        
        logger.info(f"Enrichment submitted: {len(task_ids)} tasks for content {request.content_id}")
        return response
...
async def bulk_enrich_content(
    ...
    try:
        ...
        logger.info(f"Bulk enrichment submitted: {response.successful_submissions}/{response.total_submitted} successful")
        return response
...
async def get_enrichment_analytics(
    ...
    try:
        ...
        analytics = EnrichmentAnalytics(
            period_start=datetime.fromisoformat(analytics_data["period"]["start"].replace("Z", "+00:00")),
            period_end=datetime.fromisoformat(analytics_data["period"]["end"].replace("Z", "+00:00")),
