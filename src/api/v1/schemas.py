...
class SearchRequest(BaseModel):
    ...
    technology_hint: Optional[str] = Field(
        None, description="Optional technology filter (e.g., 'python', 'react')"
    )
    limit: int = Field(
...
class SearchResult(BaseModel):
    ...
    relevance_score: float = Field(
        ..., ge=0.0, le=1.0, description="Relevance score for the query"
    )
    content_type: str = Field(
        ..., description="Type of content (documentation, code, example, etc.)"
    )
    workspace: str = Field(
        ..., description="AnythingLLM workspace containing this content"
    )
...
class SearchResponse(BaseModel):
    ...
    technology_hint: Optional[str] = Field(
        None, description="Technology filter applied"
    )
...
class FeedbackRequest(BaseModel):
    ...
    feedback_type: Literal[
        "helpful", "not_helpful", "outdated", "incorrect", "flag"
    ] = Field(..., description="Type of feedback")
    rating: Optional[int] = Field(
        None, ge=1, le=5, description="Optional 1-5 star rating"
    )
    comment: Optional[str] = Field(
        None, max_length=1000, description="Optional feedback comment"
    )
    query_context: Optional[str] = Field(
        None, description="Search query that led to this content"
    )
...
class SignalRequest(BaseModel):
    ...
    content_id: Optional[str] = Field(
        None, description="The ID of the document that was interacted with."
    )
    result_position: Optional[int] = Field(
        None, description="The 0-based index of the clicked result."
    )
    dwell_time_ms: Optional[int] = Field(
        None, description="Time in milliseconds spent on a result."
    )
...
class StatsResponse(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="Statistics timestamp"
    )
...
class Collection(BaseModel):
    ...
    is_active: bool = Field(
        True, description="Whether collection is actively maintained"
    )
...
class ConfigurationItem(BaseModel):
    ...
    is_sensitive: bool = Field(
        False, description="Whether value contains sensitive data"
    )
...
class ConfigurationUpdateRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Optional description of the change"
    )
...
class AdminContentItem(BaseModel):
    ...
    content_type: str = Field(
        ..., description="Type of content (e.g., 'documentation', 'code', 'readme')."
    )
    technology: Optional[str] = Field(
        None, description="Associated technology or framework."
    )
    source_url: Optional[str] = Field(
        None, description="Original source URL of the content."
    )
    collection_name: str = Field(
        ..., description="Name of the collection this content belongs to."
    )
    created_at: datetime = Field(..., description="Timestamp when content was indexed.")
    last_updated: datetime = Field(
        ..., description="Timestamp when content was last modified."
    )
    size_bytes: Optional[int] = Field(None, description="Size of the content in bytes.")
    status: Literal["active", "flagged", "removed"] = Field(
...
class AdminSearchResponse(BaseModel):
    ...
    items: List[AdminContentItem] = Field(
...
class ProviderResponse(BaseModel):
    ...
    category: Optional[str] = Field(
        None, description="Provider category (local, cloud, enterprise)"
    )
    description: Optional[str] = Field(None, description="Provider description")
    requires_api_key: Optional[bool] = Field(
        None, description="Whether provider requires API key"
    )
    supports_embedding: Optional[bool] = Field(
        None, description="Whether provider supports embeddings"
    )
