...
async def search_documents_get(
    ...
    return await search_documents_post(request, search_request, BackgroundTasks(), search_orchestrator)
...
async def submit_feedback(
    ...
    """
    try:
        logger.info(f"Feedback received for content {feedback_request.content_id}: {feedback_request.feedback_type}")
        
        # Add background task to process feedback (placeholder)
        def process_feedback():
            logger.info(f"Processing feedback: {feedback_request.content_id} - {feedback_request.feedback_type}")
...
async def submit_signal(
    ...
    """
    try:
        logger.info(f"Signal received: {signal_request.signal_type} for query {signal_request.query_id}")
        
        # Add background task to record signal (placeholder)
        def record_signal():
            logger.info(f"Recording signal: {signal_request.signal_type} - {signal_request.query_id}")
